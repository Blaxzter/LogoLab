...
export function useDocEdits({
  ...
  const mergeFills = useCallback(
    (edited: EditableDoc): EditableDoc => {
      ...
      for (const it of doc.items) if (it.kind === 'path') base.set(it.id, it)
      return {
        ...edited,
        items: edited.items.map((it) => {
          if (it.kind !== 'path') return it
          const b = base.get(it.id)
          return b ? { ...it, fill: b.fill, gradient: b.gradient } : it
        }),
      }
    },
    [doc, forceColorOn],
  ...
  const handlePaletteChange = useCallback(
    (palette: { r: number; g: number; b: number; a?: number }[] | null) => {
      ...
      const fo = (c: { a?: number }) => (c.a !== undefined && c.a < 255 ? c.a / 255 : undefined)
      const alphaOnly =
        !!palette &&
        !!old &&
        !!cur &&
