 ...
 */
export function DiffPane({ pz, score, primary = false }: { pz: PanZoom; score: TraceScore; primary?: boolean }) {
  ...
  const probe = useMemo(() => (cursor ? probeDiff(score, cursor.nx, cursor.ny) : null), [score, cursor])
  ...
  useEffect(() => {
    ...
    id.data.set(diffPicture(score.heat, score.de, score.source, score.width, score.height))
...
function HeatLegend({ score, probe }: { score: TraceScore; probe: DiffProbe | null }) {
  ...
  return (
    <div className="pointer-events-none absolute bottom-2 left-2 rounded-md border border-line bg-surface/85 px-2 py-1.5 font-mono text-[10px] tabular-nums text-muted backdrop-blur">
      <div>
        mean {score.meanDeltaE.toFixed(2)} · p95 {score.p95DeltaE.toFixed(2)}
      </div>
      <div className="mt-1 flex items-center gap-1.5">
        <span>0</span>
        <span className="h-2 w-24 rounded-sm" style={{ background: `linear-gradient(to right, ${ramp})` }} />
