    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (k === 'Delete' || k === 'Backspace') {
        ...
        const applyHeal = (next: EditableDoc): boolean => {
          ...
          if (!next.items.some((it) => it.id === item.id)) handleSelectPath(null)
          return true
        }
        if (item.kind === 'path' && selectedNodes.size > 0) {
          ...
          if (item.loops) {
            ...
            if (prov) {
              ...
              if (applyHeal(removeRegionSection(doc, item.id, sub))) return
          ...
          // stale.
          const seed = seedRef.current?.id === item.id ? seedRef.current.pt : null
          if (item.kind === 'path' && item.loops && seed && applyHeal(removeRegionAndHeal(doc, item.id, seed))) return
          commitDoc({
            ...doc,
            items: doc.items.filter((it) => it.id !== item.id),
          })
