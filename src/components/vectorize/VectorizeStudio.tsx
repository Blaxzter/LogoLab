...
import { Check, Copy, Download, Hand, Loader2, MousePointer2, Redo2, Undo2 } from 'lucide-react'
...
function withItem(doc: EditableDoc, item: DocItem): EditableDoc {
  return { ...doc, items: doc.items.map((it) => (it.id === item.id ? item : it)) }
}
...
export function VectorizeStudio() {
  ...
  const [retraceVector, setRetraceVector] = useState<'clean' | 'retrace'>('clean')
  ...
  const handleSelectNodes = useCallback((keys: Set<string>) => setSelectedNodes(keys), [])
  ...
  const mergeFills = useCallback(
    (edited: EditableDoc): EditableDoc => {
      ...
      for (const it of doc.items) if (it.kind === 'path') fills.set(it.id, it.fill)
      return {
        ...edited,
        items: edited.items.map((it) =>
          it.kind === 'path' && fills.has(it.id) ? { ...it, fill: fills.get(it.id)! } : it,
        ),
      }
    },
    [doc, forceColorOn],
    ...
    try {
      ...
      if (cleanFromExisting && logo.svgText) {
        ...
        next = await traceImage(
          ...
          (p) => {
            if (runId !== runIdRef.current) return
            setProgress(p.phase === 'quantize' ? 'Quantizing colors…' : `Tracing layer ${p.layer}/${p.total}…`)
          },
          controller.signal,
        )
      }
      ...
      if (runId === runIdRef.current) {
        setError('Could not vectorize this image — try different settings or another file.')
      }
    } finally {
      if (runId === runIdRef.current) {
        setBusy(false)
  ...
  const derivedDoc = useMemo(() => {
    ...
    return {
      ...doc,
      items: doc.items.map((it) => (it.kind === 'path' ? { ...it, fill: forceColor } : it)),
    }
  ...
  useEffect(() => { setApplied(false) }, [svgText])
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (k === 'Delete' || k === 'Backspace') {
        ...
        if (item.kind === 'path' && selectedNodes.size > 0) {
          const next = deleteNodes(item, [...selectedNodes].map(parseNodeKey))
          if (next) {
            commitDoc(withItem(doc, next))
            setSelectedNodes(new Set())
          } else {
            // Nothing drawable left — the whole path goes.
            commitDoc({ ...doc, items: doc.items.filter((it) => it.id !== item.id) })
            setSelectedNodes(new Set())
            handleSelectPath(null)
          }
        } else {
      ...
      if (
        ...
        commitDoc(withItem(doc, moveNodes(item, [...selectedNodes].map(parseNodeKey), dx, dy)))
      }
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [doc, selectedPathId, selectedNodes, undo, redo, commitDoc, handleSelectPath])
  ...
  const handleRecolor = useCallback(
    (id: string, fill: string, commit: boolean) => {
      ...
      const next = {
        ...doc,
        items: doc.items.map((it) => (it.id === id && it.kind === 'path' ? { ...it, fill } : it)),
      }
  ...
  const handleToggleVisible = useCallback(
    (id: string) => {
      ...
      commitDoc({
        ...doc,
        items: doc.items.map((it) => (it.id === id ? { ...it, visible: !it.visible } : it)),
      })
    },
    [doc, commitDoc],
  )
  ...
  const handleDeleteItem = useCallback(
    (id: string) => {
      if (!doc) return
      commitDoc({ ...doc, items: doc.items.filter((it) => it.id !== id) })
    ...
    <div className="flex h-full min-h-0 animate-in-fade">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex h-12 shrink-0 items-center gap-2 border-b border-line bg-surface px-3">
          ...
          />
          <div className={viewMode === 'original' ? 'pointer-events-none opacity-50' : ''}>
            <Segmented<Tool>
              value={tool}
              onChange={setTool}
              options={[
        ...
        <div className={`relative min-h-0 flex-1 ${checkerClass}`}>
          {viewMode === 'split' && (
            <div className="grid h-full grid-cols-2">
              ...
              <div className="relative h-full min-w-0">
                {derivedDoc ? (
                  <EditorCanvas {...canvasShared} doc={derivedDoc} />
                ) : (
          ...
          {viewMode === 'traced' &&
            (derivedDoc ? (
              <EditorCanvas {...canvasShared} doc={derivedDoc} primary />
            ) : (
              <StagePlaceholder busy={busy} />
            ))}
          {viewMode === 'original' && (
          ...
          {viewMode === 'overlay' &&
            (derivedDoc ? (
              <EditorCanvas
                ...
                doc={derivedDoc}
                primary
                underlay={{ src: logo.src, opacity: overlayOpacity / 100 }}
              />
            ) : (
              <StagePlaceholder busy={busy} />
            ))}
        ...
        <footer className="flex h-9 shrink-0 items-center gap-4 border-t border-line bg-surface px-3 font-mono text-xs tabular-nums text-muted">
          ...
          {error && <span className="truncate text-bad">{error}</span>}
