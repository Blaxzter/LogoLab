...
import type { EditableDoc, DocItem, NodeRef, PathItem, RawItem, Vec } from '../../lib/path/types'
...
const PathView = memo(function PathView({ item, interactive }: { item: PathItem; interactive: boolean }) {
  return (
    <path
      data-id={item.id}
      d={dOf(item)}
      ...
      style={interactive ? { pointerEvents: 'visiblePainted', cursor: 'move' } : undefined}
    />
  )
})

/** Invisible wide-stroke copy so thin outlines are grabbable in node mode. */
const HitPath = memo(function HitPath({ item, width }: { item: PathItem; width: number }) {
  return (
    <path
      data-id={item.id}
      d={dOf(item)}
...
export function EditorCanvas({
  ...
  const endMarquee = useCallback(() => {
    ...
    setMarqueeRect(null)
    if (m) {
      try { svgRef.current?.releasePointerCapture(m.pointerId) } catch { /* ok */ }
    }
    return m
  }, [])
      ...
      selectedItem.subPaths.forEach((sp, sub) =>
        sp.nodes.forEach((node, idx) => {
          ...
          for (const h of [node.hIn, node.hOut]) {
            if (h && Math.hypot(h.x - pt.x, h.y - pt.y) <= HANDLE_HIT_PX / scale) onHandle = true
          }
        }),
      )
      if (bestRef) {
        ...
        onDocCommit(withItem(doc, setNodeKind(selectedItem, ref, node.kind === 'smooth' ? 'corner' : 'smooth')))
        return
      }
  ...
  const handleSvgPointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (m) {
      if (!m.moved) {
        ...
        m.moved = true
        try { svgRef.current?.setPointerCapture(m.pointerId) } catch { /* ok */ }
      }
      const pt = toVb(e.clientX, e.clientY)
      if (!pt) return
      m.currentVb = pt
    ...
    if (drag.type === 'path') {
      ...
      item = moveHandle(drag.origItem, drag.handleRef!, drag.which!, pt, drag.mirror!)
    }
        ...
        <div ref={boxRef} className="relative" style={{ width: fit.width, height: fit.height }}>
          ...
          >
            <g onPointerDown={handlePathPointerDown}>
              {doc.items.map((item) =>
                item.kind === 'path'
                  ? item.visible && <PathView key={item.id} item={item} interactive={interactive} />
                  : item.visible && <RawView key={item.id} item={item} />,
              )}
              {interactive &&
                doc.items.map((item) =>
                  ...
                  sp.nodes.map((node, idx) => {
                    ...
                    return (
                      <g key={key}>
                        ...
                        {node.hIn && (
                          <circle cx={node.hIn.x} cy={node.hIn.y} r={r(3.25)} fill="#ffffff" stroke={ACCENT} strokeWidth={r(1.2)} />
                        )}
                        {node.hOut && (
                          <circle cx={node.hOut.x} cy={node.hOut.y} r={r(3.25)} fill="#ffffff" stroke={ACCENT} strokeWidth={r(1.2)} />
                        )}
                        {node.kind === 'smooth' ? (
                          <circle
                            cx={node.x}
