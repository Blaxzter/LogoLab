  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6">
      ...
      <p className="mt-2 max-w-[70ch] text-sm leading-relaxed text-ink-2">
        The harnesses the vectorizer is built and tested against. They run the real tracer, in your browser, on the same
        corpora and the same gates the test suite uses — so what you see here is what CI measures. Nothing is
        precomputed.
      </p>

      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        {LAB_VIEWS.map((v) => (
          <Link key={v.to} to={v.to} className="scene-card group flex gap-3 p-4 transition-colors hover:border-accent">
      ...
      <p className="mt-6 max-w-[70ch] text-xs leading-relaxed text-faint">
        Tracing is real work: a corpus takes seconds to a minute, and the labs trace one case at a time so the page
        stays responsive while it fills in.
      </p>
    </div>
  )
}
