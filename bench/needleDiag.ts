...
//
//   node --experimental-strip-types bench/needleDiag.ts                       # score + hot clusters + unmatched corners
//   node --experimental-strip-types bench/needleDiag.ts --roi x0,y0,x1,y1     # ROI autopsy: per-point dists, doc nodes, corners
//   node --experimental-strip-types bench/needleDiag.ts --matrix --roi ...    # fit-flag bisect scored on the ROI
//   node --experimental-strip-types bench/needleDiag.ts --png out.png --roi ...  # source | trace crop sheet (zoomed)
...
import type { SubPath, EditableDoc } from '../src/lib/path/types.ts'
import { DEFAULT_PLANAR_FIT, type ApexDiagRecord } from '../src/lib/trace/planarFit.ts'
