...
//
//   node --experimental-strip-types bench/stepRampDiag.ts                       # olympic-rings @512 on white, gradients ON
//   node --experimental-strip-types bench/stepRampDiag.ts --case letter-joins    # a fixture (white, like the truth gate)
//   node --experimental-strip-types bench/stepRampDiag.ts --case letter-joins --transparent   # the /labs/ab fixture-lane input
//   node --experimental-strip-types bench/stepRampDiag.ts --pair 0081c8,00a651   # the two colours to follow through the merge
//   --res N (default 512)   --all (print EVERY evaluated pair)   --jump J (counterfactual maxUnwitnessedJump)
//   node --experimental-strip-types bench/stepRampDiag.ts --census              # every accepted merge, every /labs/ab case
//
...
import type { MergePairRecord } from '../src/lib/trace/segment.ts'
import { srgbToOklab, oklabDeltaE } from '../src/lib/trace/oklab.ts'
