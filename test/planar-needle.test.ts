...
test('needle: curved-arm join corners land on their authored positions', () => {
  ...
  assert.ok(
    Math.max(...off) > 1.6,
    `precondition: the chords-only worst join should read the chord displacement (was ${Math.max(...off).toFixed(2)}px)`,
  )
  assert.ok(
    sum(on) < 11,
    `Σ over the 12 authored join corners is ${sum(on).toFixed(1)}px (chords-only: ${sum(off).toFixed(1)})`,
  )
  assert.ok(
    Math.max(...on) < 1.5,
    `the worst join corner is ${Math.max(...on).toFixed(2)}px off (chords-only: ${Math.max(...off).toFixed(2)})`,
  )
})
...
test('needle: crotch apexes come back onto the bisector', () => {
  ...
  assert.ok(
    sum(off) > 5,
    `precondition: chords-only crotch Σ should be off the bisector (was ${sum(off).toFixed(2)}px)`,
  )
  assert.ok(
    sum(on) < 4.6,
    `Σ over the 6 crotch vertices is ${sum(on).toFixed(2)}px (chords-only: ${sum(off).toFixed(2)})`,
  )
  assert.ok(
    sum(on) < sum(off) - 1.5,
    `the model must beat the chords by a clear margin (${sum(on).toFixed(2)} vs ${sum(off).toFixed(2)})`,
  )
})
...
test('needle: straight-arm geometry is byte-identical', () => {
  ...
  for (const p of NOTCH) {
    ...
    assert.ok(
      Math.abs(dOff - dOn) < 1e-9,
      `notch corner (${p.x}, ${p.y}) moved: ${dOff.toFixed(4)} → ${dOn.toFixed(4)}px`,
    )
  }
})
...
test('needle: the eroded spike keeps its reconstruction', () => {
  ...
  assert.ok(
    a <= b + 0.25,
    `the spike lost its reconstruction: ${b.toFixed(2)}px → ${a.toFixed(2)}px off its authored apex`,
  )
})
