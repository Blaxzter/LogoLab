...
function relabelMismatches(
  labels: Int32Array,
  w: number,
  h: number,
): { interior: number; bad: number; regions: number } {
  ...
  for (const Lb of order) {
    ...
    items.push({
      kind: 'path',
      id: 'r' + Lb,
      fill: `#${((1 << 24) | (r << 16) | (g << 8) | b).toString(16).slice(1)}`,
      fillRule: 'nonzero',
      subPaths,
      visible: true,
    })
...
test('planar: corner pinning is region/path-count neutral (count is topology, not the fit)', () => {
  ...
  const shape = (cornerTurnDeg: number): { regions: number; subPaths: number[] } => {
    ...
    return {
      regions: labels.length,
      subPaths: labels.map((lb) => materializeRegion(t.loopsByLabel.get(lb)!, edges).length),
    }
  }
...
test('planar: a sharp-cornered closed region keeps EXACT corners (no bevel)', () => {
  ...
  assert.ok(
    on.max < off.max - 0.4,
    `the fix sharpens vs legacy bevel (on ${on.max.toFixed(2)} < off ${off.max.toFixed(2)})`,
  )
...
test('planar: healColorSpikes fixes a mislabeled stroke pixel, spares AA + real gaps', () => {
  ...
  const labels = new Int32Array([
    ...
    0,
    2,
    0, // (2,2) is bg-labeled but coloured red below — the "spike"
    0,
    1,
    0,
    2,
  ...
  assert.equal(
    healColorSpikes(clean, cleanData, 3, 3, palette),
    clean,
    'no mislabeled pixels ⇒ returns the input array (byte-identical)',
  )
