...
function latticeLoop(inside: Inside, cx: number, cy: number, reach: number): Vec[] {
  ...
  const ink = (x: number, y: number): boolean => {
    ...
    let c = 0
    for (let a = 0; a < 4; a++)
      for (let b = 0; b < 4; b++) if (inside(ox + x + (a + 0.5) / 4, oy + y + (b + 0.5) / 4)) c++
    return c >= 8
  }
  let sx = -1
  let sy = -1
...
test('small polygons: squares ≥7px, diamonds and rotated squares keep their corners (§9.8)', () => {
  for (const s of [7, 8, 10, 12, 16]) {
    assert.equal(
      corneredAfterVeto((cx, cy) => polygon(cx, cy, s / Math.SQRT2, 4, Math.PI / 4), s),
      16,
      `${s}px square rounded`,
    )
  }
  for (const s of [5, 6, 7, 8, 10, 12, 16]) {
    assert.equal(
      corneredAfterVeto((cx, cy) => polygon(cx, cy, s / Math.SQRT2, 4, 0), s),
      16,
      `${s}px diamond rounded`,
    )
  }
  for (const rot of [0.35, 0.52])
    for (const s of [8, 10, 12]) {
      assert.equal(
        corneredAfterVeto((cx, cy) => polygon(cx, cy, s / Math.SQRT2, 4, Math.PI / 4 + rot), s),
        16,
        `${s}px square @${rot} rounded`,
      )
    }
  for (const s of [10, 12, 16]) {
    assert.equal(
      corneredAfterVeto((cx, cy) => polygon(cx, cy, s / 2, 3, -Math.PI / 2), s),
      16,
      `${s}px triangle rounded`,
    )
  }
})
...
for (const d of [9, 10, 11, 12])
  test(`mono trace: ${d}px dots come back as circles, not corner polygons`, async () => {
    ensureImageData()
    const W = 160
    const H = 60
    ...
    const doc = await traceImage({ width: W, height: H, data } as unknown as ImageData, {
